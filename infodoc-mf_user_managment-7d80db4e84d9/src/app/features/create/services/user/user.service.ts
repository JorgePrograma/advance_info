...
export class UserService {
  ...
  getUserByDocument(documentNumber: string): Observable<BasicInfoModel | null> {
    // Suponiendo que el endpoint recibe el número de documento como parámetro de consulta
    ...
    return this.http
      ...
      .pipe(
        map((response) => {
          ...
          console.log("response usuarios", response.data.items)

            return response.data.items[0];
          }
          console.log("response usuarios no encotrado", )
