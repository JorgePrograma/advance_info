...
export class ProcessTransactionCreateService {
  ...
  createUser(
    ...
    const { basicInfo, contactInfo, employeeInfo, accountInfo, signatureInfo } = formData;
    ...
    return this.validateDocumentAndUserExists(models.basicInfo.documentNumber, accountInfo.user).pipe(
      ...
      switchMap(() => {
        ...
        const fileName = `${models.basicInfo.firstName}_${models.basicInfo.documentNumber}`.toLowerCase();
        return this.avatarService.uploadAvatar(accountInfo.avatarPath, fileName).pipe(
          tap({
            next: uploadResponse => console.log('Respuesta del servicio de avatar:', uploadResponse),
      ...
      switchMap(({ basicInfoResponse, userId, models }) => {
        ...
        const updatedModels = this.updateModelsWithIds(models, personId, userId);
        ...
        if (this.hasValidSignature(signatureInfo)) {
          operations.push(
            this.handleSignatureCreation(signatureInfo, models.basicInfo, personId).pipe(
              catchError(() => of(false))
            )
          );
        }
        ...
        return forkJoin(operations).pipe(
          map(results => [results[0], results[1]] as [ResponseModel<string>, ResponseModel<string>])
        );
      }),
  ...
  private hasValidSignature(signatureInfo: SignatureInfoModel): boolean {
    return !!signatureInfo?.password?.trim() && (
  ...
  private handleCreationError(error: any): Observable<null> {
    ...
    const displayMessage = errorMessage.includes('documento') ||
    ...
    return from(SecurityUtils.hasPassword(signatureInfo.password)).pipe(
      mergeMap(passwordHash => {
        ...
        if (signatureInfo.signatureImageMechanic?.trim()) {
          signatureOperations.push(this.createSignature(
            'MECHANIC',
  ...
  private validateDocumentAndUserExists(documentNumber: string, userName: string): Observable<void> {
    return forkJoin({
